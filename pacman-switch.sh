...
_ps_chmod_switchers() {
	chmod ${1} $(find "${_ps_enabled_switchers_path}" -mindepth 1 -type f -o -type d)
}
...
_ps_action_sw_association() {
	...
	shift 1
	case "${operation}" in
		"enable"|"select"|"disable"|"reject"|"query");;
		*) _ps_error "internal error: unknown operation '${operation}' for _ps_action_sw_association"
	esac

	local old_sw_association i goal args=$(tr ' ' '\n' <<< "$@") mode=$(${_ps_automode} && echo "auto" || echo "manual") group name priority link_path root_path
	...
	while (($# >= 1)); do
		...
		local association_path="${_ps_enabled_switchers_path}/${sw_association}"
		...
		case "${operation}" in
			"enable")
			_ps_progress "${i}" "${goal}" "writing associations for ${switcher}"
			if [ "${old_sw_association}" != "${sw_association}" ]; then
			...
			"select")
			_ps_progress "${i}" "${goal}" "selecting associations for ${switcher}"
			if [ "${old_sw_association}" != "${sw_association}" ]; then
				local selected=$(tr ' ' '\n' <<< "${_ps_selected_sw[@]%:*}" | grep "^${group}:")
				if [ -n "${selected}" ]; then
					_ps_nomessage=true _ps_action_sw_association "reject" $(_ps_nomessage=true _ps_get_enabled_switcher ${selected})
				fi
			...
			"reject")
			_ps_progress "${i}" "${goal}" "rejecting associations for ${switcher}"
			if [ "${old_sw_association}" != "${sw_association}" ]; then
			...
			"query")
			if [[ ! " ${_ps_non_integrity_sw[@]} " =~ " ${1} " ]]; then
				if [[ "${old_sw_association}" != "${sw_association}" && "${mode}" = "auto" && \
...
_ps_enable() {
	...
	if [ -n "${sw_select}" ]; then
		...
		if [ -n "${sws}" ]; then
			sw_select=($(_ps_needed=true _ps_choose_sw_to_select ${sw_select[@]} $(_ps_sw_analog_by_group ${sws})))
		else
			sw_select=()
		fi
	fi
	...
	if [ -n "${sw_select}" ]; then
		_ps_commit "Selecting associations"
		_ps_action_sw_association "select" ${sw_select[@]}
	fi
}
...
_ps_disable() {
	...
	if ${_ps_automode}; then
		...
		select_sw=$(_ps_needed=true _ps_choose_sw_to_select $(_ps_sw_analog_by_group ${sws[@]}))
		if [ -n "${select_sw}" ]; then
			_ps_commit "Checking switchers for selecting"
	...
	if [ -n "${reject_sw}" ]; then
		_ps_commit "Rejecting associations"
		_ps_action_sw_association "reject" ${reject_sw}
	fi
	...
	if [ -n "${select_sw}" ]; then
		_ps_commit "Selecting associations"
		_ps_action_sw_association "select" ${select_sw}
	fi
}
...
_ps_select() {
	...
	if ${_ps_updatemode}; then
		...
		sw_select=($(_ps_choose_sw_to_select ${data_sw}))
	fi
...
_ps_reject() {
	...
	if ${_ps_reject_replace}; then
		...
		sws=$(awk -F ':' '!a[$1 ":" $2]++ {print $1 ":" $2}' <<< "${data_sw}")
		sw_select=$(_ps_choose_sw_to_select $(_ps_sw_analog_by_group ${sws}))
	...
	_ps_commit "Rejecting associations"
	_ps_action_sw_association "reject" ${data_sw}
	...
	if [ -n "${sw_select}" ]; then
		_ps_commit "Selecting alternative associations"
		_ps_action_sw_association "select" ${sw_select}
	fi
}
...
_ps_query() {
	...
	if ${_ps_query_switchers}; then
		...
		local issue_select=($(_ps_selfmode=true _ps_action_sw_association "query" $(tr ' ' '\n' <<< ${data_sw[@]} | _ps_get_selected_switchers) | sort -u))
